...
func genValueTx(nbytes int) func(int, *BlockGen) {
	return func(i int, gen *BlockGen) {
		...
		tx, _ := types.SignTx(types.NewTransaction(gen.TxNonce(benchRootAddr), toaddr, big.NewInt(1), types.Normal, gas, nil, data,nil), types.HomesteadSigner{}, benchRootKey)
		gen.AddTx(tx)
	}
}
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	if !disk {
		...
		db, err = socdb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, sochash.NewFaker(), db, b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, nil, gspec.Config, sochash.NewFaker(), vm.Config{})
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db socdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		...
		db, err := socdb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, nil, params.TestChainConfig, sochash.NewFaker(), vm.Config{})
		if err != nil {
