...
func pricedTransaction(nonce uint64, gaslimit uint64, gasprice *big.Int, key *ecdsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, common.Address{}, big.NewInt(100), types.Normal,gaslimit, gasprice, nil,nil), types.HomesteadSigner{}, key)
	return tx
}

func setupTxPool() (*TxPool, *ecdsa.PrivateKey) {
	statedb, _ := state.New(common.Hash{}, state.NewDatabase(socdb.NewMemDatabase()))
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	stdb := c.statedb
	if *c.trigger {
		c.statedb, _ = state.New(common.Hash{}, state.NewDatabase(socdb.NewMemDatabase()))
		// simulate that the new head block included tx0 and tx1
		c.statedb.SetNonce(c.address, 2)
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.Soc))
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringTransactionPoolReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.Soc))
...
func TestInvalidTransactions(t *testing.T) {
	...
	/*
	* dpos not check gas price, receive all tx
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(-1), types.Normal, 100, big.NewInt(1), nil,nil), types.HomesteadSigner{}, key)
	from, _ := deriveSender(tx)
	pool.currentState.AddBalance(from, big.NewInt(1))
...
func TestTransactionDoubleNonce(t *testing.T) {
	...
	tx1, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(0), types.Delegate,1000000, big.NewInt(1), nil,nil), signer, key)
	tx2, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(0), types.Delegate,1000000, big.NewInt(2), nil,nil), signer, key)
	tx3, _ := types.SignTx(types.NewTransaction(0, common.Address{}, big.NewInt(0), types.Delegate,1000000, big.NewInt(1), nil,nil), signer, key)
...
func TestTransactionPoolRepricing(t *testing.T) {
	...
	// Check that we can't add the old transactions back, dpos receive all tx not check price
