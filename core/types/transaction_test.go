...
var (
	emptyTx = NewTransaction(
		...
		big.NewInt(0), Normal,0, big.NewInt(0),
		nil,
		nil,
	)
...
func TestTransactionPriceNonceSort(t *testing.T) {
	...
	for start, key := range keys {
		addr := crypto.PubkeyToAddress(key.PublicKey)
		for i := 0; i < 25; i++ {
			tx, _ := SignTx(NewTransaction(uint64(start+i), common.Address{}, big.NewInt(100), Normal,100, big.NewInt(int64(start+i)), nil,nil), signer, key)
...
func TestTransactionJSON(t *testing.T) {
	...
	for i := uint64(0); i < 25; i++ {
		...
		switch i % 3 {
		case 0:
			tx = NewTransaction(i, common.Address{1}, common.Big0, Normal, 1, common.Big2, []byte("abcdef"),nil)
		case 1:
