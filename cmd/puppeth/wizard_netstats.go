...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for socstats availability")
	if infos, err := checkSocstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["socstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["socstats"] = infos.Report()
		socstats = infos.config
	}
