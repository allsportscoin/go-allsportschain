...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"socstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "socstats":
				if infos, err := checkSocstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have socstats running, ask whether to make the secret public or not
	if w.conf.socstats != "" {
		fmt.Println()
		fmt.Println("Include socstats secret on dashboard (y/n)? (default = yes)")
