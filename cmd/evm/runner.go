...
import (
	...
	"github.com/allsportschain/go-allsportschain/cmd/evm/internal/compiler"
	...
	"github.com/allsportschain/go-allsportschain/core/vm/runtime"
...
func runCmd(ctx *cli.Context) error {
	...
	if cpuProfilePath := ctx.GlobalString(CPUProfileFlag.Name); cpuProfilePath != "" {
		...
		if err != nil {
			fmt.Fprintln(ctx.App.ErrWriter, "could not create CPU profile: ", err)
			os.Exit(1)
		}
		if err := pprof.StartCPUProfile(f); err != nil {
			fmt.Fprintln(ctx.App.ErrWriter, "could not start CPU profile: ", err)
			os.Exit(1)
		}
	...
	if ctx.GlobalBool(DumpFlag.Name) {
		...
		fmt.Println(ctx.App.ErrWriter, string(statedb.Dump()))
	}

	if memProfilePath := ctx.GlobalString(MemProfileFlag.Name); memProfilePath != "" {
		f, err := os.Create(memProfilePath)
		if err != nil {
			fmt.Fprintln(ctx.App.ErrWriter, "could not create memory profile: ", err)
			os.Exit(1)
		}
		if err := pprof.WriteHeapProfile(f); err != nil {
			fmt.Fprintln(ctx.App.ErrWriter, "could not write memory profile: ", err)
			os.Exit(1)
		}
		f.Close()
	}
	...
	if ctx.GlobalBool(StatDumpFlag.Name) {
		...
		fmt.Fprintf(ctx.App.Writer, `evm execution time: %v
heap objects:       %d
allocations:        %d
total allocations:  %d
GC calls:           %d
...
`, execTime, mem.HeapObjects, mem.Alloc, mem.TotalAlloc, mem.NumGC, initialGas-leftOverGas)
	...
	if tracer == nil {
		...
		if err != nil {
			fmt.Fprintf(ctx.App.Writer, " error: %v\n", err)
		}
	}
