...
// gsoc is the official command-line client for Allsportschain.
package main
...
const (
	clientIdentifier = "gsoc" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the go-allsportschain command line interface")
	...
	nodeFlags = []cli.Flag{
		...
		utils.SochashCacheDirFlag,
		utils.SochashCachesInMemoryFlag,
		utils.SochashCachesOnDiskFlag,
		utils.SochashDatasetDirFlag,
		utils.SochashDatasetsInMemoryFlag,
		utils.SochashDatasetsOnDiskFlag,
...
func init() {
	// Initialize the CLI app and start Gsoc
	app.Action = gsoc
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2018 The go-soc Authors"
...
// gsoc is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gsoc(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	go func() {
		...
		stateReader := socclient.NewClient(rpcClient)
