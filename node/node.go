...
func (n *Node) OpenDatabase(name string, cache, handles int) (socdb.Database, error) {
	if n.config.DataDir == "" {
		return socdb.NewMemDatabase(), nil
	}
	return socdb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}
